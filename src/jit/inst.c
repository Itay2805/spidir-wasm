...
static wasm_err_t jit_wasm_shift(spidir_builder_handle_t builder, buffer_t* code, jit_context_t* ctx, jit_function_ctx_t* func, jit_label_t* label) {
    ...
    uint64_t mask_value;
    switch (opcode) {
        case 0x74 ... 0x78: type = SPIDIR_TYPE_I32; mask_value = 31; opcode -= 0x74; break;
        case 0x86 ... 0x8A: type = SPIDIR_TYPE_I64; mask_value = 63; opcode -= 0x86; break;
        default: CHECK_FAIL();
    }
    ...
    // out-of-range counts.
    spidir_value_t mask = spidir_builder_build_iconst(builder, type, mask_value);
    shift_amt = spidir_builder_build_and(builder, shift_amt, mask);
    ...
    switch (opcode) {
        ...
        // rotl
        case 3: {
            spidir_value_t bit_count = spidir_builder_build_iconst(builder, type, mask_value + 1);

            // (x << n)
            spidir_value_t left = spidir_builder_build_shl(builder, value, shift_amt);

            // (x >> (mask & (bit_count - n)))
            spidir_value_t right = spidir_builder_build_lshr(builder, value, 
                                    spidir_builder_build_and(builder, mask, 
                                        spidir_builder_build_isub(builder, bit_count, shift_amt)));

            // or the two parts
            result = spidir_builder_build_or(builder, left, right);
        } break;
        ...
        case 4: {
            ...
            // (x >> n)
            spidir_value_t left = spidir_builder_build_lshr(builder, value, shift_amt);

            // (x << (mask & (bit_count - n)))
            spidir_value_t right = spidir_builder_build_shl(builder, value, 
                                    spidir_builder_build_and(builder, mask, 
                                        spidir_builder_build_isub(builder, bit_count, shift_amt)));
...
static wasm_err_t jit_wasm_binopf(spidir_builder_handle_t builder, buffer_t* code, jit_context_t* ctx, jit_function_ctx_t* func, jit_label_t* label) {
    ...
    spidir_value_type_t type;
    switch (opcode) {
        case 0x92 ... 0x98: type = SPIDIR_TYPE_F32; opcode -= 0x92; break;
        case 0xA0 ... 0xA6: type = SPIDIR_TYPE_F64; opcode -= 0xA0; break;
        default: CHECK_FAIL();
    }
    ...
    switch (opcode) {
        ...
        case 4: kind = (type == SPIDIR_TYPE_F32) ? JIT_HELPER_F32_MIN : JIT_HELPER_F64_MIN; break;
        case 5: kind = (type == SPIDIR_TYPE_F32) ? JIT_HELPER_F32_MAX : JIT_HELPER_F64_MAX; break;
        case 6: kind = (type == SPIDIR_TYPE_F32) ? JIT_HELPER_F32_COPYSIGN : JIT_HELPER_F64_COPYSIGN; break;
        default: CHECK_FAIL();
    }

    // in some cases we use an external helper since spidir does not 
    // contain IR nodes for them
    if (kind < JIT_HELPER_COUNT) {
        spidir_funcref_t helper;
        RETHROW(jit_get_helper(ctx, kind, &helper));
        spidir_value_t args[] = { arg1, arg2 };
        result = spidir_builder_build_call(builder, helper, ARRAY_LENGTH(args), args);
    }
...
static wasm_err_t jit_wasm_unaryopi(spidir_builder_handle_t builder, buffer_t* code, jit_context_t* ctx, jit_function_ctx_t* func, jit_label_t* label) {
    ...
    switch (opcode) {
        case 0: value = spidir_builder_build_lzcount(builder, type, arg); break;
        case 1: value = spidir_builder_build_tzcount(builder, type, arg); break;
...
wasm_err_t jit_wasm_opcode(spidir_builder_handle_t builder, buffer_t* code, jit_context_t* ctx, jit_function_ctx_t* func, jit_label_t* label) {
    ...
    switch (opcode) {
        ...
        case 0x92 ... 0x98: RETHROW(jit_wasm_binopf(builder, code, ctx, func, label)); break;
        case 0x99 ... 0x9F: RETHROW(jit_wasm_unaryopf(builder, code, ctx, func, label)); break;
        case 0xA0 ... 0xA6: RETHROW(jit_wasm_binopf(builder, code, ctx, func, label)); break;
