...
inline void assertions(void) {
  _Static_assert(sizeof(float) == sizeof(uint32_t), "incompatible float type");
  _Static_assert(sizeof(double) == sizeof(uint64_t), "incompatible double type");
...
static inline void wasm_name_new_from_string(
  ...
  wasm_name_new(out, __builtin_strlen(s), s);
}

static inline void wasm_name_new_from_string_nt(
  own wasm_name_t* out, const char* s
) {
  wasm_name_new(out, __builtin_strlen(s) + 1, s);
}
